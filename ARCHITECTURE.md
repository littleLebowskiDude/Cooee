...
| `asr/onnx/` | ONNX Runtime engine, feature-gated on `onnx`: `mel.rs` (log-mel front end), `tokenizer.rs` (byte-level BPE, both directions, so the dictionary prompt works), `runtime.rs` (loads `onnxruntime.dll` and the QNN plugin once per process), `mod.rs` (encoder on the NPU with a cached compiled context, merged decoder on the CPU, greedy loop). A model *directory* selects it. |
